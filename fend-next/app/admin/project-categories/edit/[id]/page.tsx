...
const SITES = [
  { value: "ruhul-dev", label: "Ruhul Dev" },
  { value: "lisan", label: "Lisan" },
  { value: "ruhul-scholarship", label: "Ruhul Scholarship" },
  { value: "ruhul-blockchain", label: "Ruhul Blockchain" },
];
...
export default function EditProjectCategory() {
  ...
  const [site, setSite] = useState("ruhul-dev");
  ...
  const fetchCategory = async () => {
    try {
      const res = await fetch(`/api/project-categories/${categoryId}`);
      if (res.ok) {
        const category: ProjectCategory = await res.json();
        setName(category.name);
        setDescription(category.description);
        setImage(category.image || "");
        setDisplayOrder(category.displayOrder || 0);
        setSite(category.site || "ruhul-dev");
      } else {
        alert("Project category not found");
        router.push("/admin/project-categories/list");
      }
    } catch (error) {
      console.error("Failed to fetch project category:", error);
      alert("Failed to load project category");
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/project-categories/${categoryId}`, {
        ...
        body: JSON.stringify({ name, description, image, displayOrder, site }),
      });
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        alert(data.error || "Failed to update project category");
      }
    } catch (error) {
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Update category information and order position
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
              <div className="md:grid md:grid-cols-3 md:gap-6">
                <div className="md:col-span-1">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Update your project category information.
                  </p>
                </div>
