...
/* ─── Icon Registry ─── */
// Map logoKey → React component for 'icon' type logos
const ICON_REGISTRY: Record<string, React.ReactNode> = {
  ...
  ipfs: (
    <SiIpfs fontSize={60} className="text-[#c8c8c8] mx-auto py-2" />
  ),
  nodejs: (
    <FaNodeJs fontSize={60} className="text-[#c8c8c8] mx-auto py-2" />
  ),
  reactjs: (
    <FaReact fontSize={60} className="text-[#c8c8c8] mx-auto py-2" />
  ),
  php: <FaPhp fontSize={60} className="text-white mx-auto py-2" />,
  tailwindcss: (
    <SiTailwindcss fontSize={60} className="text-white mx-auto py-2" />
  ),
  mysql: (
    <SiMysql fontSize={60} className="text-[#c8c8c8] mx-auto py-2" />
  ),
  mongodb: (
    <SiMongodb fontSize={60} className="text-[#c8c8c8] mx-auto py-2" />
  ),
  docker: (
    <FaDocker fontSize={60} className="text-[#c8c8c8] mx-auto py-2" />
  ),
  aws: <FaAws fontSize={60} className="text-white mx-auto py-2" />,
  express: (
    <SiExpress fontSize={60} className="text-white mx-auto py-2" />
  ),
};

/* ─── Image Registry ─── */
// Map logoKey → <img> for 'image' type logos
const IMAGE_REGISTRY: Record<string, React.ReactNode> = {
  fabric: (
    <img
      src={fabricLogo.src}
...
/* ─── SVG Registry ─── */
// Map logoKey → inline SVG component for 'svg' type logos
const SVG_REGISTRY: Record<string, React.ReactNode> = {
  hardhat: (
    <svg
  ...
  etherjs: (
    <svg
      ...
      y="0px"
      viewBox="0 0 100 58"
      style={{ enableBackground: "new 0 0 100 58" } as React.CSSProperties}
    >
...
/* ─── Available Logo Keys (for Admin UI) ─── */
...
export const AVAILABLE_LOGOS: LogoOption[] = [
  // Icons
  { key: "ethereum", title: "Ethereum", type: "icon" },
  { key: "solana", title: "Solana", type: "icon" },
  { key: "solidity", title: "Solidity", type: "icon" },
  { key: "ipfs", title: "IPFS", type: "icon" },
  { key: "nodejs", title: "Node.js", type: "icon" },
  { key: "reactjs", title: "React", type: "icon" },
  { key: "php", title: "PHP", type: "icon" },
  { key: "tailwindcss", title: "Tailwind CSS", type: "icon" },
  { key: "mysql", title: "MySQL", type: "icon" },
  { key: "mongodb", title: "MongoDB", type: "icon" },
  { key: "docker", title: "Docker", type: "icon" },
  { key: "aws", title: "AWS", type: "icon" },
  { key: "express", title: "Express.js", type: "icon" },
  // Images
  { key: "fabric", title: "Hyperledger Fabric", type: "image" },
  { key: "metamask", title: "MetaMask", type: "image" },
  // SVGs
  { key: "hardhat", title: "Hard Hat", type: "svg" },
  { key: "etherjs", title: "Ether.js", type: "svg" },
];
...
export function renderLogo(
  ...
  if (logoType === "icon") {
    element = ICON_REGISTRY[logoKey] ?? null;
  } else if (logoType === "image") {
    element = IMAGE_REGISTRY[logoKey] ?? null;
  } else if (logoType === "svg") {
    element = SVG_REGISTRY[logoKey] ?? null;
  }

  if (!element) return null;

  // Wrap in container with customStyle
  return <div className={customStyle || ""}>{element}</div>;
}

/* ─── Preview component for Admin UI ─── */
export function LogoPreview({
  ...
  return (
    <div className="w-16 h-16 flex items-center justify-center bg-gray-100 rounded-md overflow-hidden">
      {renderLogo(logoType, logoKey, customStyle || "text-[#c8c8c8]")}
    </div>
  );
}
